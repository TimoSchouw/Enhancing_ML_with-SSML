...
class SSIF(object):
    ...
    sample : int, optional 
        The number of samples to draw from X to train each SSiTree.
        If sample is larger than the number of samples provided,
        all samples will be used for all trees (no sampling).
    nattr : int, optional
        The number of features for which the split distribution is computed.
    max_depth : int, optional
        The height limit at which the tree construction phase stops.
    n_jobs : int, optional
    ...
    """
    def __init__(self,X, Y, max_depth=None, ntrees=30, seed=42, sample=None, nattr=None, n_jobs=-1):
        self.ntrees = ntrees
        self.X = X
        self.Y = Y
        if sample != None:
            self.sample = int(sample*X.shape[0])
        else:
            self.sample = min(max(128,int(X.shape[0]/3)), X.shape[0])
        self.Trees = []
        if max_depth != None:
            self.max_depth = int(np.ceil(max_depth*2*max(np.log2(self.sample), 2)))
        else:
            self.max_depth = int(np.ceil(2*max(np.log2(self.sample), 2)))
        self.X_in=None
        if nattr != None:
            self.nattr = int(max(1,nattr))
        else:
            self.nattr = min(X.shape[1],max(3,int(X.shape[1]/5)))
        self.forest_seed = np.random.RandomState(seed=seed)
        self.c = c_factor(self.sample)
        self.Trees = Parallel(n_jobs=n_jobs)(delayed(self.buildTree)(i, self.forest_seed.randint(0,100000)) for i in range(self.ntrees))             
    
   
    def buildTree(self, i, seed):
        """
    ...
    def compute_anomaly_scores(self, X_in):
        """
        ...
        for i in range(len(X_in)):
            ...
            for j in range(self.ntrees):
                pf = PathFactor(X_in[i],self.Trees[j],self.max_depth)
                paths[j] = pf.path
